...
def createSessionFolder(subjectFolderPath,subjectSession,subjectID,temp_DicomToBids):
    ...
    for eachModality in os.listdir(subjectFolderPath):
        if "T1w_MPR_0.5" in eachModality or "mp2rage" in eachModality:
            ...
            t2wCount+=1
        elif "Fieldmap_SE_EPI_HF" in eachModality:
            fGradCount+=1
        elif "Fieldmap_SE_EPI_FH" in eachModality or "BOLD_MB2_PAT2_HF" in eachModality:
            rGradCount+=1
        elif "BOLD_PAT2_HF" in eachModality or "TEST_EPI_HF" in eachModality or "TEST2_EPI_HF" in eachModality or "BOLD_MB2_PAT2_FH" in eachModality:
            boldCount+=1
    ...
    for eachModality in os.listdir(subjectFolderPath):
        ...
        if "T1w_MPR_0.5" in eachModality or "mp2rage" in eachModality:
            ...
            fGradVar+=1
            if fGradCount>1: protocol_file.write(f'"{New_Mod_Name}":["fmap","run-{rGradVar}_magnitude1"],\n')
            else: protocol_file.write(f'"{New_Mod_Name}":["fmap","magnitude1"],\n')
        elif "FieldMap_SE_EPI_FH" in eachModality or "BOLD_MB2_PAT2_HF" in eachModality:
            rGradVar+=1
        ...
        if eachModality!=subjectSession and "gre_" not in eachModality and "localizer" not in eachModality and "Phoenix" not in eachModality:
        ...
        for sType in scanTypes:
            ...
            for nifti in niiList:
                ...
                if sType=="func" or sType=="fmap": 
                    ...
                    elif supineOrProne=="HFS":
                        refit="/opt/afni/3drefit -deoblique -orient LPS %s"%niiPath
                elif sType=="anat": 
                    if supineOrProne=="HFP":
                        refit="/opt/afni/3drefit -deoblique -orient LAS %s"%niiPath
                    elif supineOrProne=="HFS":
                        refit="/opt/afni/3drefit -deoblique -orient RIP %s"%niiPath
                print(refit)
